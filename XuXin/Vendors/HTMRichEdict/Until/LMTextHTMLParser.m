        ...
        if (attachment) {
            ...
            switch (attachment.attachmentType) {
                case LMTextAttachmentTypeImage:
                    if (attachment.userInfo == NULL || attachment.userInfo == nil) {
                        NSString *str = [NSString stringWithFormat:@"%@",attributes[@"NSAttachment"]];
                        NSArray *array = [str componentsSeparatedByString:@"\""];
                        NSString *imgStr = array[1];
                        attachment.userInfo = [NSString stringWithFormat:@"http://192.168.1.105:8080/upload/common/%@", imgStr];
                    }
                [htmlContent appendString:[NSString stringWithFormat:@"<img src=\"%@\" width=\"%f\"/>", attachment.userInfo,ScreenW-10]];
                    NSLog(@"富文本内图片路径=%@",attachment.userInfo);
                break;
                default:
                break;
            }

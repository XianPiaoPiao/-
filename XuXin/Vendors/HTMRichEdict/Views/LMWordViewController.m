...
- (void)viewDidLoad {
    ...
    NSArray *items = @[
                       [UIImage imageNamed:@"email_notification_icon"],
                       [UIImage imageNamed:@"b_icon"],
                       [UIImage imageNamed:@"export_image_icon"],
                       [UIImage imageNamed:@"hud_cancel"],
                       ];
    ...
    [self updateImgTypingAttributes];
...
- (void)layoutTextView {
    ...
    rect.size.height -= rect.origin.y + 70;
...
- (void)updateImgTypingAttributes{
...
- (void)lm_imageSettingsController:(LMImageSettingsController *)viewController insertImage:(UIImage *)image {
    ...
    NSURLSessionDataTask *task = [self.httpManager POST:userCardUploadUrl parameters:@{@"dir":@"image"} constructingBodyWithBlock:^(id<AFMultipartFormData> _Nonnull formData) {
            ...
            NSString * result = responseObject[@"isSucc"];
            
            if ([result integerValue] == YES) {
                
                [self showStaus:@"上传成功"];
                
                attachment.userInfo = responseObject[@"result"][@"url"];

...
@implementation HaiDuiIndexPath{
}
...
+ (instancetype)indexPathWithColumn:(NSInteger)column row:(NSInteger)row {
    HaiDuiIndexPath *path = [[self alloc] initWithColumn:column row:row];
    return path;
}
...
@interface HaiDuiMenu () <UITableViewDataSource, UITableViewDelegate>
{
    struct {
...
@implementation HaiDuiMenu{
    CAShapeLayer * _pricelayer;
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height andClassName:(NSString *)className{
    ...
    if (self) {
        ...
        _backGroundView.frame = CGRectMake(origin.x, origin.y + 64, SCREEN_WIDTH, SCREEN_HEIGHT);
...
- (void)setDataSource:(id<LrdSuperMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numberOfColumn; i++) {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:0 inColumn:i] > 0 && _dataSourceFlag.numberOfItemsInRow) {
            ...
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:i row:0 item:0]];
            if (i== 0) {
                if (_className.length) {
                    
                    titleString = _className;
                }
            }
        }else {
            titleString = [_dataSource menu:self titleForRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:i row:0]];
            if (i== 0) {
                if (_className.length) {
        ...
        if (i == 0) {
            ...
            [tempIndicators addObject:sharpLayer];
                        ...
                        CAShapeLayer *sharpLayer = [CAShapeLayer new];
                        [self.layer addSublayer:sharpLayer];
            ...
            _pricelayer = [CAShapeLayer new];
            _pricelayer.bounds=CGRectMake(0, 0, 12, 15);
            _pricelayer.position=CGPointMake(self.frame.size.width/4 *3.0f - 15, 22);
            _pricelayer.contents = (__bridge id _Nullable)([UIImage imageNamed:@"exchange_icon_price_top_arrow@2x"].CGImage);
            [self.layer addSublayer:_pricelayer];
            [tempIndicators addObject:_pricelayer];
            
        }else if (i == 3){
            
            CAShapeLayer *layer = [CAShapeLayer new];
            layer.bounds=CGRectMake(0, 0, 15, 15);
            layer.position=CGPointMake(self.frame.size.width - 15, 22);
            layer.contents = (__bridge id _Nullable)([UIImage imageNamed:@"exchange_icon_screen@2x"].CGImage);
...
//    layer.position=CGPointMake(self.frame.size.width/4.0f - 20, 20);
//    layer.contents = (__bridge id _Nullable)([UIImage imageNamed:@"selectCard"].CGImage);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell2) {
        ...
        cell2.textLabel.textColor = _textColor;
        cell2.textLabel.highlightedTextColor = _selectedTextColor;
        cell2.textLabel.font = [UIFont systemFontOfSize:_fontSize];
        
        if (_dataSourceFlag.detailTextForRowAtIndexPath && _dataSourceFlag.detailTextForItemInRowAtIndexPath) {
            cell2.detailTextLabel.textColor = _detailTextColor;
            cell2.detailTextLabel.font = _detailTextFont;
        }
    }
    if (tableView == _leftTableView) {
        
        if (_dataSourceFlag.titleForRowsAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];

            if (_dataSourceFlag.imageNameForRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageNameForRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            ...
            if (_dataSourceFlag.detailTextForRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            }else {
        ...
        if (_dataSourceFlag.titleForItemInRowAtIndexPath) {
            //背景颜色
            cell2.contentView.backgroundColor = [UIColor colorWithHexString:BackColor];
            NSInteger currentSelectedRow = [_currentSelectedRows[_currentSelectedColumn] integerValue];

           cell2.textLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            
          
            if (_dataSourceFlag.imageNameForItemInRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageForItemsInRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
                if (imgName && imgName.length > 0) {
                    cell.imageView.image = [UIImage imageNamed:imgName];
                }else {
                    cell.imageView.image = nil;
            ...
            if (_dataSourceFlag.detailTextForItemInRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForItemsInRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            }else {
                cell.textLabel.text = nil;
            }
        }
        if ([cell2.textLabel.text isEqualToString:[(CATextLayer *)_titles[_currentSelectedColumn] string]]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _leftTableView) {
        ...
        if (isClickHaveItemValid && _delegate &&[_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            ...
            [_delegate menu:self didSelectRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
        }
    }else {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:[_currentSelectedRows[_currentSelectedColumn] integerValue] item:indexPath.item]];
        }
    }
}
...
- (void)selectDeafultIndexPath {
    [self selectIndexPath:[HaiDuiIndexPath indexPathWithColumn:0 row:0]];
}
//获取IndexPath所对应的字符串
...
- (void)selectIndexPath:(HaiDuiIndexPath *)indexPath {
    ...
    if (indexPath.item < 0 ) {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:indexPath.row inColumn:indexPath.column] > 0){
            title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
                [_delegate menu:self didSelectRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:indexPath.column row:indexPath.row item:0]];
        }else {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
...
- (void)menuTapped:(UITapGestureRecognizer *)gesture {
    ...
    if (touchIndex == _currentSelectedColumn && _isShow) {
        ...
        _currentSelectedColumn = touchIndex;
        if (touchIndex > 0) {
            NSString * str =[NSString stringWithFormat:@"%ld",touchIndex + 1];
            NSDictionary * info = [NSDictionary dictionaryWithObjectsAndKeys:str,@"one", nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"orderBy" object:nil userInfo:info];
        }
        ...
        if (touchIndex == 0) {
            ...
            [_rightTableView removeFromSuperview];
            [self animateIndicator:nil background:nil tableView:nil title:_titles[touchIndex] reverse:YES complecte:^{
                _isShow = YES;
            }];
            ...
            if (i == 0) {
                ...
                _pricelayer.contents = (__bridge id _Nullable)([UIImage imageNamed:@"exchange_icon_price_bottom_arrow@3x"].CGImage);
                i++;
            }else if ( i == 1){
            ...
            NSDictionary * info = [NSDictionary dictionaryWithObjectsAndKeys:str,@"one", nil];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"priceDesc" object:nil userInfo:info];
...
- (BOOL)setMenuWithSelectedRow:(NSInteger)row {
    ...
    if (_dataSourceFlag.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row inColumn:_currentSelectedColumn] > 0) {
        if (_isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:row]];
...
- (void)setMenuWithSelectedItem:(NSInteger)item {
    ...
    title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[HaiDuiIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedMenudRow item:item]];

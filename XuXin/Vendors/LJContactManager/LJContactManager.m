...
- (void)createNewContactWithPerson:(BusinessCardModel *)cardModel controller:(UIViewController *)controller{
    if (IOS9_OR_LATER)
    {
        CNMutableContact *contact = [[CNMutableContact alloc] init];
        contact.familyName = [cardModel.username substringToIndex:1];
        contact.givenName = [cardModel.username substringWithRange:NSMakeRange(1, cardModel.username.length-1)];
        contact.nickname = cardModel.username;
        //电话,每一个CNLabeledValue都是有讲究的，如何批评，可以在头文件里面查找，这里是给出几个常用的
        CNLabeledValue *phoneNumber = [CNLabeledValue labeledValueWithLabel:CNLabelPhoneNumberMobile value:[CNPhoneNumber phoneNumberWithStringValue:cardModel.mobile]];
        contact.phoneNumbers = @[phoneNumber];
        
        //网址:
        CNLabeledValue *url = [CNLabeledValue labeledValueWithLabel:CNLabelURLAddressHomePage value:cardModel.web];
        //邮箱:
        CNLabeledValue *mail = [CNLabeledValue labeledValueWithLabel:CNLabelWork value:cardModel.email];
    ...
    else
    {
        ...
        ABMutableMultiValueRef multiValue = ABMultiValueCreateMutable(kABMultiStringPropertyType);
        CFErrorRef error = NULL;
        ABMultiValueAddValueAndLabel(multiValue, (__bridge CFTypeRef)(cardModel.mobile), kABPersonPhoneMobileLabel, NULL);
        ABRecordSetValue(newPerson, kABPersonPhoneProperty, multiValue , &error);
        CFRelease(multiValue);
        picker.displayedPerson = newPerson;
        CFRelease(newPerson);

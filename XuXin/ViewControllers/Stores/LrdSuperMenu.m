...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height andClassName:(NSString *)className {
    ...
    if (self) {
        ...
        _rightTableView.backgroundColor = [UIColor colorWithHexString:BackColor];
    ...
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                ...
                _rightTableView.frame = CGRectMake(self.origin.x + SCREEN_WIDTH / 2 -1, self.origin.y +self.height, SCREEN_WIDTH / 2, tableViewHeight);

//
//  SBMyOrderTableviewController.m
//  XuXin
//
...
@interface SBMyOrderTableviewController ()
...
@implementation SBMyOrderTableviewController{
    BOOL isUseRedwallet;
}
...
-(void)junmAnother:(UIButton *)btn{
    if (self.weixinBtn.selected == YES) {
        ...
        CGFloat price =  [_orderPrice floatValue] - _sendFee ;
...
-(void)creatNavgationBar{
  ...
  //  UIBarButtonItem * temporaryBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"fanhui"] style:UIBarButtonItemStylePlain target:self action:@selector(returnAction)];
...
-(void)requestWeixinData{
    ...
    [weakself.httpManager POST:payOrderUrl parameters:param progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        [SVProgressHUD showErrorWithStatus:@"亲，你的网络不行,请稍后再试"];
...
- (void)doAlipayPay
{
    ...
    order.biz_content.total_amount = [NSString stringWithFormat:@"%.2f", [_orderPrice floatValue]]; //商品价格
